...
const OverviewPage: React.FC<OverviewPageProps> = ({
  ...
  const getThemeStyles = (): {
    ...
    if (theme === "light") {
      return {
        ...
        cardClickableClass:
          "cursor-pointer hover:shadow-lg hover:border-orange-500 border border-transparent transition-all duration-200",
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:5001/api/projects/${projectId}/events`
        );
  ...
  const findClosestUpcomingEvent = (
    eventList: EventData[]
  ): EventData | null => {
    const now = new Date();
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6">
        ...
        {overviewItems.map((item, idx) => (
          ...
          >
            <div className={`text-4xl ${styles.iconColor} mb-4`}>
              {item.icon}
            </div>
            <h3 className={`text-xl font-semibold mb-2 ${styles.cardText}`}>
              {item.label}
            </h3>
            {item.label === "Events" ? (
              isLoadingEvents ? (
        ...
        >
          ...
          {isLoadingMembers ? (
            ...
            <div className="flex justify-center -space-x-2 overflow-hidden">
              ...
              {members.slice(0, 5).map(renderMemberAvatar)}{" "}
