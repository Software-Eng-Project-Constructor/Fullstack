...
function getFileIcon(name: string): JSX.Element | null { // Changed return type
...
const FileManagement: React.FC<FileManagementProps> = ({ projectId }) => {
            ...
            try {
                ...
                if (Array.isArray(parsedFiles)) {
                    const validatedFiles: FileItem[] = parsedFiles.map((file: any) => ({
                        ...
                        lastUpdated: typeof file.lastUpdated === 'string' ? file.lastUpdated : '',
                        icon: file.icon && React.isValidElement(file.icon) ? file.icon : null, // IMPORTANT: Validate icon
                        fileUrl: typeof file.fileUrl === 'string' ? file.fileUrl : undefined,
                    }));
    ...
    useEffect(() => {
        if (files.length > 0) { // only save if there are files.
                            ...
                            <div className={`${styles.previewBg} p-4 rounded-lg mb-4 border ${styles.inputBorder}`}>
                                <div className="text-center">
                                    {previewModal.file.type.match(/^(PNG|JPG|JPEG|GIF)$/) ? (
                                        ...
                                        <div className="py-8">
                                            <span className="text-4xl mb-2">{previewModal.file.icon || <FaFileAlt/>}</span>
